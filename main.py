...
# Build the Flask app and monitoring clients from environment-based settings.
app = create_app()
...
def record_request(response):
    # Record every request after the route has produced its response.
    ...
    if response.status_code >= 500:
        # Notify operators about server failures without exposing credentials.
...
def aws_identity():
    ...
    # This endpoint is protected because the AWS identity is operational data.
    try:
